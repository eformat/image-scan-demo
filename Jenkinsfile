            ...
            timeout(time: 2, unit: 'DAYS') {
                def userInput = input(
                    id: 'userInput', message: 'CVE\'s found, Continue ?', parameters: [
                        [$class: 'BooleanParameterDefinition', defaultValue: false, description: '', name: 'continue']]) 
            }
        }
    }
...
def containsCVE() {
    ...
    def cmd7 = $/curl -H 'Accept: text/plain'  https://security.sensiolabs.org/check_lock -F lock=@composer.lock 2>&1/$
    try {
